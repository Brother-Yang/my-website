...
export class BaseEnvironment extends ModelsLoad {
  ...
  initLights1() {
    {
      const hemisphereLight = new THREE.HemisphereLight(0xffffff, 0xeeeeee, 0.4);
  ...
  useSkyEnvironment(env?: boolean) {
    ...
    if (env) {
      ...
      this.scene.environment = this.pmremGenerator.fromScene(sky as unknown as THREE.Scene).texture;
    }
    return sky.material;
  }
  ...
  protected initRenderer(parameters?: IProps) {
    ...
    const pixelRatio = parameters?.limitPixelRatio ? windowPix : Math.min(windowPix, 2);
